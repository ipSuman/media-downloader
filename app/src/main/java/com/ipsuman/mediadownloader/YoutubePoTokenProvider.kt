...
class YoutubePoTokenProvider(private val context: Context) {
    ...
    @Volatile private var tokenTimestamp = 0L
    ...
    private fun importCookiesIntoWebView() {
        ...
        try {
            val cm = CookieManager.getInstance(); cm.setAcceptCookie(true)
            file.forEachLine { raw ->
                val line = raw.trim()
                if (line.isEmpty() || line.startsWith("#")) return@forEachLine
                val parts = line.split('\t')
                if (parts.size < 7) return@forEachLine
                val domain = parts[0]; val path = parts[2].ifEmpty { "/" }; val name = parts[5]
    ...
    fun onRunBotguardResult(botguardResponse: String) {
        Thread {
            try {
                ...
                val parsed = JSONArray(response); val integrityToken = base64ToJsU8(parsed.getString(0)); val ttl = parsed.optLong(1, 3600L)
                mainHandler.post {
                    try {
                        webView?.evaluateJavascript("this.integrityToken = $integrityToken") {
                            integrityTokenReady = true; webPoSignalReady = true; initialized = true; initializing = false
                        }
                    } catch (e: Exception) { failInitialization(e.message ?: e.toString()) }
                }
    ...
    private fun runBotguardPreparation() { try { webView?.evaluateJavascript("typeof runBotGuard === 'function'") {} } catch (_: Exception) {} }
    private fun failInitialization(message: String) { lastError = message; initializing = false; initialized = false }
    ...
    private fun makeIntegrityRequest(urlString: String, data: List<String>): String {
        ...
        connection.setRequestProperty("User-Agent", USER_AGENT); connection.setRequestProperty("Accept", "application/json"); connection.setRequestProperty("Content-Type", "application/json+protobuf")
        connection.setRequestProperty("x-goog-api-key", GOOGLE_API_KEY); connection.setRequestProperty("x-user-agent", "grpc-web-javascript/0.1")
        val body = JSONArray().apply { data.forEach { put(it) } }.toString(); connection.outputStream.use { it.write(body.toByteArray(StandardCharsets.UTF_8)) }
    ...
    private fun getVisitorData(): String {
        val body = JSONObject().apply { put("context", JSONObject().apply { put("client", JSONObject().apply { put("clientName", WEB_CLIENT_NAME); put("clientVersion", WEB_CLIENT_VERSION) }) }) }.toString()
        val connection = URL("https://www.youtube.com/youtubei/v1/visitor_id?prettyPrint=false").openConnection() as HttpURLConnection
        connection.requestMethod = "POST"; connection.connectTimeout = 15000; connection.readTimeout = 20000; connection.doOutput = true
        connection.setRequestProperty("User-Agent", USER_AGENT); connection.setRequestProperty("Accept", "application/json"); connection.setRequestProperty("Content-Type", "application/json")
        connection.setRequestProperty("X-YouTube-Client-Name", WEB_CLIENT_ID); connection.setRequestProperty("X-YouTube-Client-Version", WEB_CLIENT_VERSION)
        ...
        lastVisitorData = visitor; lastError = "Visitor data acquired from Innertube"; return visitor
    }
    ...
    private fun parseChallengeData(raw: String): String {
        ...
        val safe = findFirstString(challengeData.optJSONArray(1)); val trusted = findFirstString(challengeData.optJSONArray(2))
        return JSONObject().apply {
            put("messageId", challengeData.getString(0)); put("interpreterJavascript", JSONObject().apply { put("privateDoNotAccessOrElseSafeScriptWrappedValue", safe ?: JSONObject.NULL); put("privateDoNotAccessOrElseTrustedResourceUrlWrappedValue", trusted ?: JSONObject.NULL) })
            put("interpreterHash", challengeData.getString(3)); put("program", challengeData.getString(4)); put("globalName", challengeData.getString(5)); put("clientExperimentsStateBlob", challengeData.getString(7))
        }.toString()
    }

    private fun findFirstString(array: JSONArray?): String? { if (array == null) return null; for (i in 0 until array.length()) if (array.opt(i) is String) return array.getString(i); return null }
    private fun descramble(value: String): String { val bytes = decodeYoutubeBase64(value); for (i in bytes.indices) bytes[i] = (bytes[i].toInt() + 97).toByte(); return String(bytes, StandardCharsets.UTF_8) }
    private fun base64ToJsU8(value: String): String { val bytes = decodeYoutubeBase64(value); return "new Uint8Array([${bytes.joinToString(",") { (it.toInt() and 0xff).toString() }}])" }
    private fun decodeYoutubeBase64(value: String): ByteArray { var normalized = value.replace('-', '+').replace('_', '/').replace('.', '='); while (normalized.length % 4 != 0) normalized += "="; return Base64.decode(normalized, Base64.DEFAULT) }

    fun getMwebGvsToken(timeoutSeconds: Long = 25): String? {
        val cached = cachedResult
        if (cached != null && System.currentTimeMillis() - tokenTimestamp < TOKEN_TTL_MS) { lastVisitorData = cached.visitorData; lastError = "mweb GVS PO Token cache hit"; return cached.poToken }
        return generateFreshToken(timeoutSeconds)?.poToken
    }

    private fun waitForInitialization(timeoutSeconds: Long): Boolean {
        val deadline = System.nanoTime() + TimeUnit.SECONDS.toNanos(timeoutSeconds)
        while (System.nanoTime() < deadline) {
            if (initialized) return true
            if (!initializing) mainHandler.post { startInitialization() }
            Thread.sleep(50)
        }
        return initialized
    }
    ...
    private fun generateFreshToken(timeoutSeconds: Long): PoTokenResult? {
        if (!waitForInitialization(timeoutSeconds)) { lastError = lastError ?: "Android BotGuard provider initialization timed out"; return null }
        val visitorData = try { getVisitorData() } catch (e: Exception) { lastError = "Could not obtain YouTube visitorData: ${e.message}"; return null }
        synchronized(tokenLatchLock) { tokenResults.remove(visitorData); tokenErrors.remove(visitorData) }
        mainHandler.post {
            try {
            ...
            } catch (e: Exception) { synchronized(tokenLatchLock) { tokenErrors[visitorData] = "PO-token JavaScript failed: ${e.message}" } }
        }
        val deadline = System.nanoTime() + TimeUnit.SECONDS.toNanos(timeoutSeconds)
        while (System.nanoTime() < deadline) {
            synchronized(tokenLatchLock) {
                tokenResults[visitorData]?.let {
                    val result = PoTokenResult(it, visitorData); cachedResult = result; tokenTimestamp = System.currentTimeMillis(); lastVisitorData = visitorData; lastError = "mweb GVS PO Token generated successfully"; return result
                }
                tokenErrors[visitorData]?.let { lastError = "PO-token generation failed: $it"; return null }
            }
            Thread.sleep(50)
        }
        lastError = "PO-token generation timed out"; return null
    }

    /** Clear the cached pair and start a completely fresh WebView/BotGuard session. */
    fun invalidateToken() {
        cachedResult = null; tokenTimestamp = 0L; lastVisitorData = null
        synchronized(tokenLatchLock) { tokenResults.clear(); tokenErrors.clear() }
        mainHandler.post {
            try { webView?.stopLoading(); webView?.destroy() } catch (_: Exception) {}
            webView = null; initialized = false; initializing = false; webPoSignalReady = false; integrityTokenReady = false
            lastError = "PO-token cache invalidated; starting fresh WebView authentication"
            startInitialization()
        }
    }

    /** Invalidate and synchronously wait for a new token pair. */
    fun refreshToken(timeoutSeconds: Long = 25): PoTokenResult? { invalidateToken(); return generateFreshToken(timeoutSeconds) }
    ...
    @JavascriptInterface fun onObtainPoTokenError(identifier: String, error: String) { synchronized(tokenLatchLock) { tokenErrors[identifier] = error } }
    fun lastError(): String? = lastError
    fun close() { mainHandler.post { try { webView?.stopLoading(); webView?.destroy() } catch (_: Exception) {}; webView = null } }
}
